...
public class Exam6 {
	// c언어 : 함수명은 중복되면 안됨
	static int plus_int_int(int x, int y) {
		return x + y;
	}

	static double plus_int_double(int x, double y) {
		return x + y;
	}

	static double plus_double_int(double x, int y) {
		return x + y;
	}

	static double plus_double_double(double x, double y) {
		return x + y;
	}

	// java : 매개변수의 자료형과 개수가 다를 경우 함수명을 똑같이 써도 된다.
